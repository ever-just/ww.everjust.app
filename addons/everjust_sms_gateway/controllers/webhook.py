...
class SmsWebhookController(http.Controller):
    ...
    @http.route("/sms/incoming", type="json", auth="none", csrf=False, methods=["POST"])
    def incoming_sms(self):
        ...
        if not phone or not body:
            return {"status": "ignored", "reason": "missing phone or body"}
        ...
        if partner:
            ...
            _logger.info("SMS from %s → partner %s (#%s)", phone, partner.name, partner.id)
        else:
            _logger.info("SMS from unknown number %s: %s", phone, body[:80])

        return {"status": "ok", "partner_id": partner.id if partner else None}
